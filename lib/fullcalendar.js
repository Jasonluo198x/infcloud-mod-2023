...
function BasicView(element, calendar, viewName) {
	...
	function updateCells(firstTime) {
		...
		bodyCells.each(function(i, _cell) {
			...
			var lStr = lunarMonthDayStr(date,true,false);
			if (lStr != ''){
				cell.find('div.fc-day-text').text(lStr);		
			}
...
function lunarMonthDayStr(date,daysonly,withbrackets) {
	if (!globalSettings.showextracal.value){
		return '';
	}
	var lStr = "";
	...
	if (daysonly) {
		if (lunar.day == 1) {
			lStr = (lunar.isLeap?"闰":"") + monthName[lunar.month-1];
		}
		else {
			if (lunar.day == 20) {
				lStr = "二十";
				...
				lStr = nStr2[parseInt((lunar.day - 1) / 10)] + nStr1[(lunar.day-1) % 10];
			}
		}
	}
	else {
...
function AgendaDayView(element, calendar) {
	...
	function render(date, delta) {
		...
		t.title = formatDate(date, opt('titleFormat')) + lunarMonthDayStr(date,false,true);
...
function AgendaView(element, calendar, viewName) {
	...
	function updateCells() {
		...
		for (i=0; i<colCnt; i++) {
			...
			headCell.html(formatDate(date, colFormat) + lunarMonthDayStr(date,false,true));
	...
	function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
		...
		for (i=0; i<segCnt; i++) {
			...
			if (!event.allDay && seg.isStart && opt('timeFormat')) {
				...
				html +=
                                        "<span class='fc-event-time'>" +
                                        htmlEscape(formatDates(event.start, event.end, opt('timeFormat'))).split("-")[0] +
                                        "</span>";
		...
		for (i=0; i < events.length; i++) {
			...
			for(j = 0; j <= dayDuration; j++) {
				...
				if (segmode == 'smart') {
					...
					segHash = formatDate(segDate, colFormat) + lunarMonthDayStr(segDate,false,true);
				} else {
					segHash = '';
				}
...
function TableView(element, calendar) {
	...
	function buildSkeleton(date) {
		...
		if(opt('showDatepicker')) {
			...
			dateInfoText.html(formatDates(origDate, null, opt('titleFormat', 'table')) + lunarMonthDayStr(date,false,true));
...
function TodoEventRenderer() {
	...
	function compileSegs(events) {
		...
		for (i=events.length-1; i > -1; i--) {
			...
			// skip events out of range
			if ((!globalSettings.todolistshowallcompleted.value && event.completedOn && event.completedOn < t.start 
				&& (opt('showUnstartedEvents') || !event.start || event.completedOn > event.start)) 
